-- 04 — Kitchen/staff handovers + branch-to-branch store transfers
-- Endesha baada ya 03. Haina data yoyote.
...
DROP TRIGGER IF EXISTS handovers_updated ON public.handovers;
CREATE TRIGGER handovers_updated BEFORE UPDATE ON public.handovers
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

CREATE INDEX IF NOT EXISTS handovers_org_created_idx
  ON public.handovers (org_id, created_at DESC);
...
GRANT SELECT, INSERT, UPDATE ON public.store_transfers TO authenticated;
GRANT ALL ON public.store_transfers TO service_role;
ALTER TABLE public.store_transfers ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "transfers_select" ON public.store_transfers;
CREATE POLICY "transfers_select" ON public.store_transfers FOR SELECT TO authenticated
  USING (
    ...
    OR public.is_org_member(auth.uid(), to_org_id)
  );
DROP POLICY IF EXISTS "transfers_insert" ON public.store_transfers;
...
DROP POLICY IF EXISTS "transfers_update" ON public.store_transfers;
...
CREATE OR REPLACE FUNCTION public.create_store_transfer(
  _from_org_id UUID, _to_org_id UUID, _product_id UUID, _qty NUMERIC, _note TEXT DEFAULT NULL
) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE
  _uid UUID := auth.uid();
  _p RECORD; _before NUMERIC; _after NUMERIC; _id UUID;
BEGIN
  ...
  _before := _p.stock; _after := _before - _qty;
...
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE
  _uid UUID := auth.uid(); _t RECORD; _dest UUID; _before NUMERIC; _after NUMERIC;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
...
DECLARE
  _uid UUID := auth.uid(); _t RECORD; _before NUMERIC; _after NUMERIC;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
  SELECT * INTO _t FROM public.store_transfers WHERE id = _transfer_id FOR UPDATE;
  IF _t IS NULL THEN RAISE EXCEPTION 'transfer not found'; END IF;
